...
Adapted from https://github.com/KyleKing/mdformat-plugin-template.

Repos to check are configured in 'scripts/canary_repos.json'. Every entry is a
repository that pins 'mdformat-footnote' in its own tooling and has markdown
containing footnote definitions, so a regression here is a regression someone
downstream would hit. JSON (not a Python module of 'Repo(...)' calls) so the
'Repo' shape can change without a migration; unknown or missing fields are
ignored or defaulted rather than raising.

To add or update an entry, run
`git -C .tox/canary/cache/<name> show HEAD:.pre-commit-config.yaml` and check
for a mdformat hook plus its args/excludes, then mirror them in the JSON entry
so canary tracks what the downstream repo actually formats. Only 'name' and
'url' are required.
...
settings too, under 'plugin.footnote'::

    "options": {"plugin": {"footnote": {"keep_orphans": true}}}

'extensions' names other mdformat plugins the repo's own docs build depends on
(e.g. 'mkdocs' for MkDocs admonitions), on top of the 'footnote' this project
always tests. Add the matching PyPI package to the canary tox env's deps in
tox.ini.
"""
...
_DEFINITION_RE = re.compile(r"^ {0,3}\[\^([^\]\s]+)\]:", re.MULTILINE)
...
def _lost_footnotes(original: str, formatted: str) -> tuple[str, ...]:
    """Labels that were referenced and defined in the original, but lost a definition.

    Unreferenced definitions are excluded because dropping those is the
    documented default behavior (see '--keep-footnote-orphans').
    """
    expected = set(_REFERENCE_RE.findall(original)) & set(
        _DEFINITION_RE.findall(original)
    )
    return tuple(sorted(expected - set(_DEFINITION_RE.findall(formatted))))
...
class FileResult:
    ...
    diff: str | None = None
    new_escapes: int = 0
    lost_footnotes: tuple[str, ...] = ()

    @property
    def passed(self) -> bool:
        """True if the file produced no errors, no diff, and no lost footnotes.

        'new_escapes' is a warning surfaced separately, not a failure.
        """
        return self.error is None and self.diff is None and not self.lost_footnotes
...
class CheckResult:
    ...
    def output(self) -> str:
        ...
        for result in self.file_results:
            ...
            elif result.lost_footnotes:
                labels = ", ".join(f"[^{lbl}]" for lbl in result.lost_footnotes)
                lines.extend((f"Lost footnotes: {result.path}", f"  {labels}"))
            elif result.diff:
...
_EXTENSIONS = {"footnote"}


def _load_repos(path: Path) -> list[Repo]:
    """Parse 'canary_repos.json', defaulting fields this version doesn't know."""
...
def _clone_or_pull(repo: Repo, target_dir: Path) -> None:
    ...
    # Leading slash anchors to the repo root; without it git warns that a
    # bare 'README.md' matches at any depth.
    anchored = [f"/{pattern}" for pattern in repo.patterns]
    subprocess.run(
        ["git", "sparse-checkout", "set", "--no-cone", *anchored],
        cwd=target_dir,
        check=True,
    )
...
def _collect_files(repo: Repo, target_dir: Path) -> list[Path]:
    ...
    # Sparse checkout leaves symlinks pointing at paths it did not materialize
    return sorted(path for path in included - excluded if path.is_file())
...
def _check_file(
    ...
    shared = {
        ...
        "lost_footnotes": _lost_footnotes(original, pass1),
    }
    if pass1 == pass2:
        return FileResult(path=path, **shared)
    ...
    return FileResult(path=path, diff=diff, **shared)
...
def main(argv: list[str]) -> None:
    ...
    if not all_repos:
        print("No canary repos configured in scripts/canary_repos.json. Skipping.")
        return
